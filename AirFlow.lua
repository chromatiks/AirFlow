        ...
        function f:LoadFont(b)
            ...
            local function af(a)
                ...
                local function x(d, e, f, g, h)
                    local b = c("ImageLabel", {AnchorPoint = Vector2.new(.5, .5), Position = f, Size = e, BackgroundTransparency = 1, Image = t.Glow, ImageColor3 = a.Accent, ImageTransparency = g, ZIndex = 0, Parent = d}) c("UIGradient", {Color = ColorSequence.new(Color3.fromRGB(255, 255, 255), a.Accent), Rotation = h or 90, Parent = b})
                    return b
                end
                local function M(a)
                                ...
                                for a, b in pairs(table.clone(j)) do
                                    ...
                                    local function aj()
                                        ...
                                        function f.Window(ab, k)
                                            ...
                                            local n = c(H, {Name = "Body", Size = UDim2.fromScale(1, 1), BackgroundColor3 = a.Background, BorderSizePixel = 0, Parent = z}) j.Body = n e(n, UDim.new(0, 10)) j.BodyStroke = h(n, a.Stroke) M(n) x(n, UDim2.fromOffset(560, 220), UDim2.new(.5, 0, 1, 10), .78, 270) x(n, UDim2.fromOffset(200, 120), UDim2.new(0, -20, 1, -20), .7, 90) x(n, UDim2.fromOffset(480, 260), UDim2.new(1, -10, 0, 0), .84, 90) x(n, UDim2.fromOffset(160, 160), UDim2.new(0, 40, 0, 40), .88, 45) x(n, UDim2.fromOffset(300, 140), UDim2.new(0, 85, 1, -30), .82, 0)
                                            ...
                                            j._keyChipLabel = Y do
                                                local rebinding = false y.MouseButton1Click:Connect(function() if rebinding then
                                                        return
                                                    end
                                                    rebinding = true Y.Text = "..." Y.TextColor3 = a.Accent local conn conn = q.InputBegan:Connect(function(inp) if inp.UserInputType ~= Enum.UserInputType.Keyboard then
                                                            return
                                                        end
                                                        if inp.KeyCode == Enum.KeyCode.Escape then
                                                            Y.Text = P(j.Keybind) Y.TextColor3 = a.Muted rebinding = false conn:Disconnect() return
                                                        end
                                                        j:SetKeybind(inp.KeyCode) Y.TextColor3 = a.Muted rebinding = false conn:Disconnect() j:Notify {Title = "Menu bind", Content = "Set to " .. P(inp.KeyCode), Duration = 2}
                                                    end
                                                    )
                                                end
                                                )
                                            ...
                                            table.insert(j._connections, q.InputBegan:Connect(function(a, b) if b then
                                                ...
                                                function j:SetWatermark(t, s)
                                                    if j._wmTitle and t ~= nil then
                                                        j._wmTitle.Text = tostring(t)
                                                    end
                                                    if j._wmSub and s ~= nil then
                                                        j._wmSub.Text = tostring(s)
                                                    end
                                                end
